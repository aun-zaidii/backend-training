...
class UserRegistrationView(APIView):
    ...
    def post(self, request:Request) ->Response:
        try:
            ...
            serializer.is_valid()
            serializer.save()
            send_mail("Registeration MAil", "You are registered successfully", "aun.zaidi@evolvyxlabs.com", [data['email']], fail_silently=False,)
...
class UserUpdate(APIView):
    ...
    def put(self,request:Request)->Response:
        try:
            ...
            serializer.save()
            return Response({'success':'User Updated'}, status=status.HTTP_204_NO_CONTENT)
        except ValidationError as e:
            return Response ({'failed':str(e)}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as  e:
            return Response({'failed':str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
